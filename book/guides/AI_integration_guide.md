...
Cloudbank sponsors Claude Code usage of up to ~$20 per day per participant during the workshop. To use the sponsored Claude Code resource, a consent form must be signed, and a key needs to be assigned to you. You should have been reached out by Anshul Tambay regarding these. Here we discuss how to set up the AI integration once these prerequisites are met.

## Jupyter-ai

Our JupyterLab is built with the `jupyter-ai` extension included. This creates a "chat" tab on the left panel, as well as a chat launcher on the launch page. The "chat" tab turns the left panel into a chat space, while the chat launcher turns the main panel into a chat space. You can switch between the two panels using the "move" icons at the top of the respective panels.

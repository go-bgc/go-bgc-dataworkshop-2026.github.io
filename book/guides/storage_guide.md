...
In general, the long-term persistence of any of the storage spaces listed below is not guaranteed. Make sure you backup all data relevant to you as the workshop comes to an end. 
...
+ The `shared` directory under the home directory (`/home/jovyan/shared`) is read-only and has a quota of 100 GB. This is the place where you'll find pre-loaded workshop tutorial/demo data. All participants will have read-only access to this folder and will see the same contents. If you believe that there are additional data that should be accessible to all participants, contact Wing-Ho Ko, who have write access to the folder.

+ The rest of the home directory (`/home/jovyan`, mapped as the root on the file browser on the left) is accessible only by you and has a quota of 10 GB. This is a good place to store your codes and "small" datasets. This directory is persistent across different server instances.
...
Importantly, **all** participants have read and write interface to **all** files on the bucket. As basic etiquette, do not erase other people's data. In addition, your data should always be stored inside a subfolder ("prefix" in Amazon s3 lingo). For your own data, use your GitHub username as the name of the subfolder (this is automatically set as the `SCRATCH_BUCKET` variable in your home environment). For project data, derive the subfolder name from the project name.

To read/write data on the bucket, you can either use the Terminal interface (via `aws s3` command line tool) or through python.

### Terminal interface

In the Terminal interface, you can use the `aws s3` command line tool to manipulate files on the s3 bucket. The interface somewhat resembles the classic Linux file operation interface. In particular,

+ `aws s3 ls <remote_path>` list the contents under a particular s3 path.
...
+ `aws s3 rm <remote_path>` removes a file located at a particular s3 path.
...
The python package `s3fs` can be used manipulate files on the s3 bucket. It provides both a python interface (resembling the built-in `open()`) and Linux-like interface.

The first step for manipulating s3 bucket is to start a s3 client:
